...
export default function LoginButton() {
  ...
  if (status === "loading") {
    return (
      <div className="h-10 w-28 bg-coco-accent/20 animate-pulse border-2 border-coco-dark/10" />
    );
  }
      ...
      <div className="flex items-center gap-3">
        {session.user?.image && (
          <img
            ...
            alt="Avatar"
            className="w-8 h-8 border-2 border-coco-accent"
          />
        )}
        <span className="text-sm text-coco-cream/80 hidden sm:inline font-medium">
          {session.user?.name}
        </span>
        <button
          onClick={() => signOut()}
          className="px-3 py-1.5 text-sm bg-coco-dark text-coco-cream border-2 border-coco-coffee hover:bg-coco-coffee transition-colors font-medium"
        >
          Sign Out
        </button>
      </div>
  ...
  return (
    <button
      ...
      className="btn-discord flex items-center gap-2 text-sm"
    >
