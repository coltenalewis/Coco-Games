...
export default function ConnectionStatus({
  ...
  return (
    <div className="mt-8 flex flex-wrap gap-3">
      {/* Discord Status */}
      <div className="flex items-center gap-2.5 px-4 py-2.5 bg-[#5865F2]/15 border border-[#5865F2]/30 backdrop-blur-sm">
        ...
        <span className="text-sm text-coco-cream font-medium">{discordName}</span>
        <span className="text-[10px] text-green-400 font-bold uppercase tracking-wider">Connected</span>
      </div>

      {/* Roblox Status */}
      <Link
        href={robloxLinked ? "/profile" : "/profile"}
        className={`flex items-center gap-2.5 px-4 py-2.5 border backdrop-blur-sm transition-all ${
          robloxLinked
      ...
      >
        ...
        <div className="w-4 h-4 bg-[#E2231A] flex items-center justify-center">
          <span className="text-white font-black text-[8px]">R</span>
        </div>
        <span className="text-sm text-coco-cream font-medium">Roblox</span>
        {robloxLinked ? (
          <span className="text-[10px] text-green-400 font-bold uppercase tracking-wider">Connected</span>
        ) : (
          <span className="text-[10px] text-coco-cream/40 font-bold uppercase tracking-wider">Not linked</span>
        )}
      </Link>
    </div>
  );
